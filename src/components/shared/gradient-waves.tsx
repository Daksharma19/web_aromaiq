...
// Adapted from React Bits (reactbits.dev) GradientWaves.
...
const GradientWaves: React.FC<GradientWavesProps> = ({
  ...
  // Colors ease toward these targets each frame, so prop changes cross-fade instead of snapping.
  const colorTargetRef = useRef<{ h: number[]; w: number[]; c: number[]; initialized: boolean }>({
    h: [1, 1, 1],
    w: [1, 1, 1],
    c: [1, 1, 1],
    initialized: false
  ...
  useEffect(() => {
    ...
    // The shaders are GLSL 300 es; ogl silently falls back to WebGL1, where they would fail to compile.
    ...
    // A fresh program starts white; let the props effect snap colors again (e.g. after a Strict Mode remount).
    colorTargetRef.current.initialized = false;
    ...
    // Listen on window: the canvas usually sits behind content, so it would rarely receive pointer events itself.
    ...
    const loop = (t: number) => {
      ...
      m[0] = currentMouse[0];
      m[1] = currentMouse[1];
      const targets = colorTargetRef.current;
      const pairs: [Float32Array, number[]][] = [
        [(program.uniforms.uHorizonColor as { value: Float32Array }).value, targets.h],
        [(program.uniforms.uWaveColor as { value: Float32Array }).value, targets.w],
        [(program.uniforms.uCrestColor as { value: Float32Array }).value, targets.c]
      ];
      for (const [color, target] of pairs) {
        for (let i = 0; i < 3; i++) color[i] += 0.04 * (target[i] - color[i]);
      }
      renderer.render({ scene: mesh });
  ...
  useEffect(() => {
    ...
    const targets = colorTargetRef.current;
    targets.h = hexToRgb(horizonColor);
    targets.w = hexToRgb(waveColor);
    targets.c = hexToRgb(crestColor);
    if (!targets.initialized) {
      (u.uHorizonColor.value as Float32Array).set(targets.h);
      (u.uWaveColor.value as Float32Array).set(targets.w);
      (u.uCrestColor.value as Float32Array).set(targets.c);
